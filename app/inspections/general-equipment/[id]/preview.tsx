...
import { INSPECTION_TYPE_LABEL } from '../../../../types/generalEquipment';
import type { GeneralEquipmentInspection } from '../../../../types/generalEquipment';

export default function GeneralEquipmentPreviewScreen() {
  ...
  useEffect(() => {
    let cancelled = false;
    generalEquipmentApi.getById(id)
      .then(data => {
        if (cancelled) return;
        if (data) {
          setInspection(data);
  ...
  const updateField = useCallback(
    async (field: keyof GeneralEquipmentInspection, value: unknown) => {
      if (!inspection) return;
      setInspection(prev => prev ? { ...prev, [field]: value } : prev);
      await generalEquipmentApi.update(inspection.id, { [field]: value });
    },
    [inspection],
  );
  ...
  const handleSignature = useCallback(
    async (base64: string) => {
      if (!inspection) return;
      setShowSig(false);
      const { data, error } = await generalEquipmentApi.uploadSignature(inspection.id, base64);
      if (error || !data?.path) {
  ...
  const filledCount = equipment.filter(e => e.condition !== null).length;
  const totalCount = equipment.length;
  const goodCount = equipment.filter(e => e.condition === 'good').length;
  const warnCount = equipment.filter(e => e.condition === 'needs_service').length;
  const badCount = equipment.filter(e => e.condition === 'unusable').length;

  const isApproved = !equipment.some(e => e.condition === 'unusable');
    ...
    <Screen>
      ...
      <SafeAreaView style={styles.root} edges={['top', 'bottom']}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle} numberOfLines={1}>აღჭურვილობა</Text>
          <View style={{ width: 24 }} />
        </View>
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.pdfRow}>
            ...
            <View style={styles.pdfTitleBlock}>
              ...
              <Text style={styles.pdfId}>{inspection.actNumber || inspection.id.slice(0, 8).toUpperCase()}</Text>
            </View>
          </View>
          ...
          <View style={styles.infoCard}>
            <View style={styles.infoRow}>
              <View style={styles.infoCol}>
                ...
                <Text style={styles.infoValue}>{inspection.objectName || '—'}</Text>
              </View>
              <View style={styles.infoCol}>
                <Text style={styles.infoLabel}>სახე</Text>
                <Text style={styles.infoValue}>{inspection.inspectionType ? INSPECTION_TYPE_LABEL[inspection.inspectionType] : '—'}</Text>
              </View>
            </View>
            <View style={styles.infoRow}>
              <View style={styles.infoCol}>
              ...
              <View style={styles.infoCol}>
                <Text style={styles.infoLabel}>აღჭურვილობა</Text>
                <Text style={styles.infoValue}>{totalCount} ერთ.</Text>
              </View>
            </View>
          </View>
          ...
          <View style={[styles.statusCard, isApproved ? styles.statusApproved : styles.statusRejected]}>
            ...
            <Text style={styles.statusText}>
              {isApproved ? '✓ უსაფრთხო ექსპლუატაცია' : '✗ ექსპლუატაცია აკრძალულია'}
            </Text>
          </View>
          ...
          <View style={styles.progressCard}>
            ...
            <View style={styles.progressRow}>
              <Text style={styles.progressNumber}>{String(filledCount).padStart(2, '0')}</Text>
              <Text style={styles.progressTotal}> / {totalCount} ერთ.</Text>
            </View>
            <View style={styles.progressCounts}>
              <View style={styles.countPill}>
                <Ionicons name="checkmark" size={14} color={theme.colors.semantic.success} />
                <Text style={[styles.countPillText, { color: theme.colors.semantic.success }]}>{goodCount} სარეკ.</Text>
              </View>
              <View style={styles.countPill}>
                <Ionicons name="warning" size={14} color={theme.colors.warn} />
                <Text style={[styles.countPillText, { color: theme.colors.warn }]}>{warnCount} ხარვ.</Text>
              </View>
              <View style={styles.countPill}>
                <Ionicons name="close" size={14} color={theme.colors.danger} />
                <Text style={[styles.countPillText, { color: theme.colors.danger }]}>{badCount} გამოუს.</Text>
              </View>
            </View>
          </View>
      ...
      <SignatureCanvas
        visible={showSig}
        personName={inspection.objectName || ''}
