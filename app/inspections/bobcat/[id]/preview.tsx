...
export default function BobcatPreviewScreen() {
  ...
  const updateField = useCallback(
    async (field: keyof BobcatInspection, value: unknown) => {
      if (!inspection) return;
      setInspection(prev => prev ? { ...prev, [field]: value } : prev);
      await bobcatApi.update(inspection.id, { [field]: value });
    },
    [inspection],
  );
  ...
  const handleSignature = useCallback(
    async (base64: string) => {
      if (!inspection) return;
      setShowSig(false);
      const { data, error } = await bobcatApi.uploadSignature(inspection.id, base64);
      if (error || !data?.path) {
  ...
  const counts = {
    good: inspection.items?.filter(i => i.result === 'good').length || 0,
    deficient: inspection.items?.filter(i => i.result === 'deficient').length || 0,
    unusable: inspection.items?.filter(i => i.result === 'unusable').length || 0,
    total: inspection.items?.length || 0,
  };
    ...
    <Screen>
      ...
      <SafeAreaView style={styles.root} edges={['top', 'bottom']}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle} numberOfLines={1}>დიდი დამტვირთველი</Text>
          <View style={{ width: 24 }} />
        </View>
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.pdfRow}>
            ...
            <View style={styles.pdfTitleBlock}>
              <Text style={styles.pdfTitle} numberOfLines={2}>
                დიდი დამტვირთველი — შემოწმების აქტი
              </Text>
              <Text style={styles.pdfId}>{inspection.id.slice(0, 8).toUpperCase()}</Text>
            </View>
          </View>
          ...
          <View style={styles.infoCard}>
            <View style={styles.infoRow}>
              <View style={styles.infoCol}>
                <Text style={styles.infoLabel}>მოდელი</Text>
                <Text style={styles.infoValue}>{inspection.equipmentModel || '—'}</Text>
              </View>
              <View style={styles.infoCol}>
                <Text style={styles.infoLabel}>ობიექტი</Text>
                <Text style={styles.infoValue}>{inspection.company || '—'}</Text>
              </View>
            </View>
            <View style={styles.infoRow}>
              <View style={styles.infoCol}>
              ...
              <View style={styles.infoCol}>
                <Text style={styles.infoLabel}>სახ. / ს.ნ</Text>
                <Text style={styles.infoValue}>{inspection.registrationNumber || '—'}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={[styles.statusCard, isApproved ? styles.statusApproved : styles.statusRejected]}>
            ...
            <Text style={styles.statusText}>
              {isApproved ? '✓ უსაფრთხო ექსპლუატაცია' : inspection.verdict === 'limited' ? '⚠ ზღვარზე' : '✗ ექსპლუატაცია აკრძალულია'}
            </Text>
          </View>
      ...
      <SignatureCanvas
        visible={showSig}
        personName={inspection.equipmentModel || ''}
