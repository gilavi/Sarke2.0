import{b2 as i,b5 as k}from"./index-8VpAInaW.js";const w=`
...
<body>
  ...
  `}function v(t){return!t.label&&!t.value?"":`<span class="lbl">${i(t.label)}</span><span class="val">${i(t.value)||"-"}</span>`}function A(t,a){let n="",e=0;for(;e<a.length;){const s=a[e];if(s.full){n+=`<tr><td colspan="2">${v(s)}</td></tr>`,e+=1;continue}const r=e+1<a.length&&!a[e+1].full?a[e+1]:null;n+=`<tr><td>${v(s)}</td><td>${r?v(r):""}</td></tr>`,e+=r?2:1}return`
    ${m(t)}
  ...
  `}const B={good:"ck-good",warn:"ck-def",bad:"ck-bad",neutral:""},N={good:"dot-good",warn:"dot-def",bad:"dot-bad"};function _(t){return`<div class="legend">${t.map(n=>`<span class="legend-item"><span class="dot ${N[n.tone]??""}"></span>${i(n.text)}</span>`).join("")}</div>`}function I(t,a){if(!t||t.length===0)return"";const n=t.map(e=>{const s=a[e];return s?`<span class="item-photo"><img src="${i(s)}" alt="ფოტო" /></span>`:""}).join("");return n?`<div style="margin-top:4px;">${n}</div>`:""}function P(t,a,n){const e=t.resultOptions,s=2+e.length,r="შემოწმების პუნქტი",p=t.layout==="checks"?e.map(l=>`<th class="col-check">${i(l.short??l.label)}</th>`).join(""):'<th class="col-result">შედეგი</th>';let d="";for(const l of a){l.title&&(d+=`<tr class="cl-sec-row"><td colspan="${t.layout==="checks"?s:3}">${i(l.title)}</td></tr>`);for(const o of l.items){const c=o.comment?`<div class="item-comment">${i(o.comment)}</div>`:"",h=o.description?`<div style="color:var(--inkSoft);font-size:10px;margin-top:2px;">${i(o.description)}</div>`:"",f=I(o.photoPaths,n),x=`
        <td>
          ...
          ${h}${c}${f}
        </td>`;if(t.layout==="checks"){const b=e.map(g=>o.result!==g.value||!g.mark?'<td class="col-check"></td>':`<td class="col-check"><span class="${B[g.tone??"neutral"]??""}">${g.mark}</span></td>`).join("");d+=`<tr><td class="col-num">${i(String(o.id))}</td>${x}${b}</tr>`}else d+=`<tr><td class="col-num">${i(String(o.id))}</td>${x}<td class="col-result">${M(e,o.result)}</td></tr>`}}return`
    ${m(t.title)}
    ${t.legend?_(t.legend):""}
