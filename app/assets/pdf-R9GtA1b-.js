import{aV as i,aX as k}from"./index-l9sIBqZg.js";const w=`
...
`,S="შიდა სამსახურებრივი დოკუმენტი",F=["იანვარი","თებერვალი","მარტი","აპრილი","მაისი","ივნისი","ივლისი","აგვისტო","სექტემბერი","ოქტომბერი","ნოემბერი","დეკემბერი"];function z(t){const a=new Date(t);return Number.isNaN(a.getTime())?"":`${a.getDate()} ${F[a.getMonth()]} ${a.getFullYear()}`}function j(t){if(!t)return"";const a=!!t.creatorSignature,n=Math.max(0,t.additionalRowsCount|0);if(!a&&n===0)return"";const e="ხელმოწერები",s=a?C(t.creatorSignature):"",r=n>0?D(n):"";return`
  ...
  `}function D(t){const a=[];for(let n=0;n<t;n+=1)a.push(`
    ...
    `);return a.join("")}function V(t,a,n){const{inspection:e,projectName:s,signaturesSession:r=null}=a,{docId:p,docDate:d}=t.meta(e),l=u(t.docTitle,e)??"",o=u(t.docSubtitle,e),c=u(t.pdfFooterLabel,e)??"",h=u(t.internalBadge,e)??S,f=t.headerMetaLines?t.headerMetaLines(e):[],x=t.blocks.map(y=>H(y,e,n)).filter(Boolean).join(`
`),v=j(r),g=`${w}
...
<body>
  ...
  `}function H(t,a,n){switch(t.kind){case"machineSpecs":return L(t.title,t.specs(a));case"infoFields":return A(t.title,t.fields(a));case"checklist":return P(t,t.sections(a),n);case"maintenance":return O(t,t.rows(a));case"verdict":return U(t,a,n);case"signatures":return G(t.title,t.lines(a));case"custom":return t.render(a,n);default:return t}}function m(t){return`<div class="section-title">${i(t)}</div>`}function L(t,a){const n=a.map(s=>`<th>${i(s.label)}</th>`).join(""),e=a.map(s=>`<td>${i(s.value)||"-"}</td>`).join("");return`
    ${m(t)}
    <table class="specs-table">
      <thead><tr>${n}</tr></thead>
      <tbody><tr>${e}</tr></tbody>
    </table>
  `}function b(t){return!t.label&&!t.value?"":`<span class="lbl">${i(t.label)}</span><span class="val">${i(t.value)||"-"}</span>`}function A(t,a){let n="",e=0;for(;e<a.length;){const s=a[e];if(s.full){n+=`<tr><td colspan="2">${b(s)}</td></tr>`,e+=1;continue}const r=e+1<a.length&&!a[e+1].full?a[e+1]:null;n+=`<tr><td>${b(s)}</td><td>${r?b(r):""}</td></tr>`,e+=r?2:1}return`
    ${m(t)}
    <table class="info-table">${n}</table>
  `}const B={good:"ck-good",warn:"ck-def",bad:"ck-bad",neutral:""},N={good:"dot-good",warn:"dot-def",bad:"dot-bad"};function _(t){return`<div class="legend">${t.map(n=>`<span class="legend-item"><span class="dot ${N[n.tone]??""}"></span>${i(n.text)}</span>`).join("")}</div>`}function I(t,a){if(!t||t.length===0)return"";const n=t.map(e=>{const s=a[e];return s?`<span class="item-photo"><img src="${i(s)}" alt="ფოტო" /></span>`:""}).join("");return n?`<div style="margin-top:4px;">${n}</div>`:""}function P(t,a,n){const e=t.resultOptions,s=2+e.length,r="შემოწმების პუნქტი",p=t.layout==="checks"?e.map(l=>`<th class="col-check">${i(l.short??l.label)}</th>`).join(""):'<th class="col-result">შედეგი</th>';let d="";for(const l of a){l.title&&(d+=`<tr class="cl-sec-row"><td colspan="${t.layout==="checks"?s:3}">${i(l.title)}</td></tr>`);for(const o of l.items){const c=o.comment?`<div class="item-comment">${i(o.comment)}</div>`:"",h=o.description?`<div style="color:var(--inkSoft);font-size:10px;margin-top:2px;">${i(o.description)}</div>`:"",f=I(o.photoPaths,n),x=`
        <td>
          <strong>${i(o.label)}</strong>
          ${h}${c}${f}
        </td>`;if(t.layout==="checks"){const v=e.map(g=>o.result!==g.value||!g.mark?'<td class="col-check"></td>':`<td class="col-check"><span class="${B[g.tone??"neutral"]??""}">${g.mark}</span></td>`).join("");d+=`<tr><td class="col-num">${i(String(o.id))}</td>${x}${v}</tr>`}else d+=`<tr><td class="col-num">${i(String(o.id))}</td>${x}<td class="col-result">${M(e,o.result)}</td></tr>`}}return`
    ${m(t.title)}
    ${t.legend?_(t.legend):""}
