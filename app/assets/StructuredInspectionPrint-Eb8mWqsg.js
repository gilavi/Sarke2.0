import{aq as L,ah as B,H as I,r as w,Z as D,k as g}from"./vendor-CbD0vSgE.js";import{aJ as n,aK as H,aI as T,av as N,j as _,p as R}from"./index-B8_0eqqT.js";import"./rnw-YboY0qRB.js";import"./threejs-CSPAcMbP.js";import"./supabase-DAYk6AcI.js";import"./icons-PG6dT_uj.js";import"./leaflet-C7rqjzBo.js";const M=`
...
`,O="შიდა სამსახურებრივი დოკუმენტი",K=["იანვარი","თებერვალი","მარტი","აპრილი","მაისი","ივნისი","ივლისი","აგვისტო","სექტემბერი","ოქტომბერი","ნოემბერი","დეკემბერი"];function U(t){const e=new Date(t);return Number.isNaN(e.getTime())?"":`${e.getDate()} ${K[e.getMonth()]} ${e.getFullYear()}`}function q(t){if(!t)return"";const e=!!t.creatorSignature,i=Math.max(0,t.additionalRowsCount|0);if(!e&&i===0)return"";const a="ხელმოწერები",s=e?G(t.creatorSignature):"",l=i>0?W(i):"";return`
  ...
  `}function G(t){const e=U(t.capturedAtIso);return`
  ...
  `}function W(t){const e=[];for(let i=0;i<t;i+=1)e.push(`
    ...
    `);return e.join("")}function Q(t,e,i){const{inspection:a,projectName:s,signaturesSession:l=null}=e,{docId:p,docDate:o}=t.meta(a),c=S(t.docTitle,a)??"",r=S(t.docSubtitle,a),d=S(t.pdfFooterLabel,a)??"",h=S(t.internalBadge,a)??O,v=t.headerMetaLines?t.headerMetaLines(a):[],f=t.blocks.map(F=>J(F,a,i)).filter(Boolean).join(`
`),y=q(l),u=`${M}
...
<body>
  ...
  `}function J(t,e,i){switch(t.kind){case"machineSpecs":return Z(t.title,t.specs(e));case"infoFields":return X(t.title,t.fields(e));case"checklist":return it(t,t.sections(e),i);case"maintenance":return rt(t,t.rows(e));case"verdict":return ot(t,e,i);case"signatures":return lt(t.title,t.lines(e));case"custom":return t.render(e,i);default:return t}}function b(t){return`<div class="section-title">${n(t)}</div>`}function Z(t,e){const i=e.map(s=>`<th>${n(s.label)}</th>`).join(""),a=e.map(s=>`<td>${n(s.value)||"-"}</td>`).join("");return`
    ${b(t)}
    <table class="specs-table">
      <thead><tr>${i}</tr></thead>
      <tbody><tr>${a}</tr></tbody>
    </table>
  `}function j(t){return!t.label&&!t.value?"":`<span class="lbl">${n(t.label)}</span><span class="val">${n(t.value)||"-"}</span>`}function X(t,e){let i="",a=0;for(;a<e.length;){const s=e[a];if(s.full){i+=`<tr><td colspan="2">${j(s)}</td></tr>`,a+=1;continue}const l=a+1<e.length&&!e[a+1].full?e[a+1]:null;i+=`<tr><td>${j(s)}</td><td>${l?j(l):""}</td></tr>`,a+=l?2:1}return`
    ${b(t)}
  ...
  `}function st(t,e){const i=t.find(l=>l.value===e);if(!i)return'<span class="pill pill-null">-</span>';const a=i.tone??"neutral";return`<span class="pill ${a==="good"?"pill-good":a==="warn"?"pill-def":a==="bad"?"pill-bad":"pill-null"}">${i.mark?`${i.mark} `:""}${n(i.label)}</span>`}function rt(t,e){const i=e.map(a=>{const s=a.answer==="yes"?'<span class="ck-good">✓</span>':"",l=a.answer==="no"?'<span class="ck-bad">✗</span>':"";return`
      <tr>
        ...
        <td class="col-maint-date">${a.date?n(H(a.date)):""}</td>
      </tr>`}).join("");return`
    ${b(t.title)}
  ...
  `}function ht({actKey:t}){var z,E;const{id:e}=L(),[i]=B(),a=i.get("preview")==="1",l=((z=I().state)==null?void 0:z.signaturesSession)??null,p=w.useRef(null),o=T(t),c=D({queryKey:o?o.descriptor.detailKey(e):["structured-print-missing",e],queryFn:()=>o.descriptor.get(e),enabled:!!o&&!!e}),r=c.data??null,d=r&&o?o.descriptor.getProjectId(r):null,h=D({queryKey:R.detail(d),queryFn:()=>_(d),enabled:!!d}),[v,f]=w.useState({}),[y,u]=w.useState(!1);w.useEffect(()=>{if(!o||!r)return;const x=o.schema.collectPhotoPaths(r);let m=!1;return Promise.all(x.map(async $=>{try{return[$,await N($)]}catch{return[$,""]}})).then($=>{if(m)return;const P={};for(const[A,C]of $)C&&(P[A]=C);f(P),u(!0)}),()=>{m=!0}},[o,r]);const k=c.isSuccess&&(h.isSuccess||!d)&&y;if(!o)return g.jsx("p",{style:{padding:24},children:"უცნობი შემოწმების ტიპი."});if(c.isLoading)return g.jsx("p",{style:{padding:24},children:"იტვირთება…"});if(!r)return g.jsx("p",{style:{padding:24},children:"აქტი ვერ მოიძებნა."});if(!k)return g.jsx("p",{style:{padding:24},children:"იტვირთება…"});const F=Q(o.schema,{inspection:r,projectName:((E=h.data)==null?void 0:E.name)??"",signaturesSession:l},v);return g.jsxs(g.Fragment,{children:[g.jsxs("div",{style:{position:"sticky",top:0,background:"#FAFAFA",borderBottom:"1px solid #E5E7EB",padding:"10px 16px",display:"flex",gap:8,justifyContent:"flex-end",zIndex:10},children:[g.jsx("button",{onClick:()=>window.history.back(),style:{padding:"6px 14px",borderRadius:6,fontSize:13,fontWeight:600,cursor:"pointer",border:"1px solid #D1D5DB",background:"#fff"},children:"დახურვა"}),g.jsx("button",{onClick:()=>{var x,m;return(m=(x=p.current)==null?void 0:x.contentWindow)==null?void 0:m.print()},style:{padding:"6px 14px",borderRadius:6,fontSize:13,fontWeight:600,cursor:"pointer",border:"1px solid #2F855A",background:"#2F855A",color:"#fff"},children:"ბეჭდვა"})]}),g.jsx("iframe",{ref:p,srcDoc:F,style:{width:"100%",height:"calc(100vh - 53px)",border:"none",display:"block"},title:o.descriptor.title,onLoad:()=>{var x,m;a||(m=(x=p.current)==null?void 0:x.contentWindow)==null||m.print()}})]})}export{ht as default};
