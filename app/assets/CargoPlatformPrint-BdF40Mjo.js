...
`;function Y(r){const{inspection:e,projectName:p="პროექტი",photoUrls:c}=r,n=e.id.slice(-8).toUpperCase(),g=v(e.completedAt??e.inspectionDate),u=`
    <div class="header">
  ...
  `;function x(t,a,s){return t==="none"?`☑ ${a} &nbsp; ☐ ${s}`:t==="complete"?`☐ ${a} &nbsp; ☑ ${s}`:"—"}function m(t){return t==="non_standard"?"☑ ვერ აკმაყოფილებს სტანდარტს &nbsp; ☐ სტანდარტს აკმაყოფილებს":t==="standard"?"☐ ვერ აკმაყოფილებს სტანდარტს &nbsp; ☑ სტანდარტს აკმაყოფილებს":"—"}const w=`
  ...
  `;function l(t){return t==="good"?'<span class="pill pill-good">✓ კარგი</span>':t==="fix"?'<span class="pill pill-fix">✗ გამოსასწ.</span>':t==="na"?'<span class="pill pill-na">— N/A</span>':'<span class="pill pill-null">—</span>'}let o="",I="";for(const t of G){t.section!==I&&(I=t.section,o+=`
        <tr class="cl-cat-row">
          <td colspan="4">${i(K[t.section])}</td>
        </tr>
      `);const a=e.items.find(S=>S.id===t.id),s=(a==null?void 0:a.result)??null,z=(a==null?void 0:a.comment)??null,T=(a==null?void 0:a.photo_paths)??[],L=s==="fix";let $="";for(const S of T){const j=c[S];j&&($+=`<span class="item-photo"><img src="${j}" alt="ფოტო" /></span>`)}o+=`
      <tr${L?' class="item-fix"':""}>
        <td class="col-num">${t.id}</td>
        <td>
          <strong>${i(t.label)}</strong><br>
          <span style="color:var(--inkSoft)">${i(t.description)}</span>
          ${z?`<div class="item-comment">${i(z)}</div>`:""}
          ${$?`<div style="margin-top:4px;">${$}</div>`:""}
        </td>
        <td class="col-result">${l(s)}</td>
      </tr>
    `}const P=`
  ...
  `,E=t=>e.verdict===t?"selected":"",B=t=>e.verdict===t?"checked":"",H=`
    <div class="section-title">V — დასკვნა</div>
    <div class="verdict-block">
      ${["approved","conditional","rejected"].map(t=>`
        <div class="verdict-option ${E(t)}">
          <div class="verdict-box ${B(t)}"></div>
          <span class="verdict-label">${i(M[t])}</span>
        </div>
      `).join("")}
    </div>
    <div class="comment-label" style="margin-top:14px;">კომენტარი</div>
    <div class="comment-block">${i(e.verdictComment)||"&nbsp;"}</div>
  `,C=e.summaryPhotos.map((t,a)=>{const s=c[t];return s?`
      <div class="photo-item">
        <img src="${s}" alt="ფოტო ${a+1}" />
        <div class="photo-caption">ფოტო ${a+1}</div>
      </div>
    `:""}).filter(Boolean).join(""),D=e.summaryPhotos.length>0?`
    <div class="section-title">VI — ფოტო / ვიდეო მასალა</div>
    <p style="font-size:10px;color:var(--inkSoft);font-style:italic;margin-bottom:8px;">
      დოკუმენტს თან ერთვის ტესტირების ამსახველი ფოტო/ვიდეო მასალა
    </p>
    <div class="photo-grid">${C}</div>
  `:"";function F(t,a){const s=t.signature?`<img class="sig-img" src="data:image/png;base64,${t.signature}" alt="ხელმოწერა" />`:'<div class="sig-line"></div>';return`
      <div class="sig-block">
        <div class="sig-cell">
    ...
    <div class="sig-two-col">
      ${F(e.signatures[0],"I ხელმომწერი")}
      ${F(e.signatures[1],"II ხელმომწერი")}
    </div>
...
</html>`}function ot(){var f,y,h;const{id:r}=N(),[e]=V(),p=e.get("preview")==="1",c=b.useRef(null),n=A({queryKey:["cargoPlatformInspection",r],queryFn:()=>W(r),enabled:!!r}),g=A({queryKey:["project",(f=n.data)==null?void 0:f.projectId],queryFn:()=>q(n.data.projectId),enabled:!!((y=n.data)!=null&&y.projectId)}),[u,k]=b.useState({}),[x,m]=b.useState(!1);if(b.useEffect(()=>{if(!n.data)return;const l=[...n.data.items.flatMap(o=>o.photo_paths??[]),...n.data.summaryPhotos];if(!l.length){m(!0);return}Promise.all(l.map(async o=>[o,await U(o)])).then(o=>{k(Object.fromEntries(o)),m(!0)}).catch(()=>m(!0))},[n.data]),!n.data)return d.jsx("p",{style:{padding:24},children:n.isLoading?"იტვირთება…":"ვერ მოიძებნა."});if(!g.isSuccess||!x)return d.jsx("p",{style:{padding:24},children:"იტვირთება…"});const w=Y({inspection:n.data,projectName:(h=g.data)==null?void 0:h.name,photoUrls:u});return d.jsxs(d.Fragment,{children:[d.jsxs("div",{style:{position:"sticky",top:0,background:"#FAFAFA",borderBottom:"1px solid #E5E7EB",padding:"10px 16px",display:"flex",gap:8,justifyContent:"flex-end",zIndex:10},children:[d.jsx("button",{onClick:()=>window.history.back(),style:{padding:"6px 14px",borderRadius:6,fontSize:13,fontWeight:600,cursor:"pointer",border:"1px solid #D1D5DB",background:"#fff"},children:"დახურვა"}),d.jsx("button",{onClick:()=>{var l,o;return(o=(l=c.current)==null?void 0:l.contentWindow)==null?void 0:o.print()},style:{padding:"6px 14px",borderRadius:6,fontSize:13,fontWeight:600,cursor:"pointer",border:"1px solid #2F855A",background:"#2F855A",color:"#fff"},children:"ბეჭდვა"})]}),d.jsx("iframe",{ref:c,srcDoc:w,style:{width:"100%",height:"calc(100vh - 53px)",border:"none",display:"block"},title:"ტვირთის მიმღები პლატფორმის შემოწმების აქტი",onLoad:()=>{var l,o;p||(o=(l=c.current)==null?void 0:l.contentWindow)==null||o.print()}})]})}export{ot as default};
