 ...
 *
 * Behavior-faithful replacement for lib/bobcatPdf.ts. Covers both template
 * variants that share the `bobcat_inspections` table — "ციცხვიანი დამტვირთველი"
 * (bobcat / skid-steer) and "დიდი ციცხვიანი დამტვირთველი" (large loader) —
 * distinguished at render time by `inspection.templateId`. The body layout
 * (category column, result pills, per-category summary table, the neutral
 * "not present" pill) diverges from the typed-block shape, so its sections are
 ...
 *
 * Variant handling: `docTitle`/`docSubtitle`/`pdfFooterLabel` are functions of
 * `inspection.templateId`, so the large-loader variant ("დიდი ციცხვიანი
 * დამტვირთველი") gets its own header title/subtitle/footer — matching the
 * original builder. Body variant logic (item catalog, category counts) is
...
/** Catalog selection — mirrors the old builder / screen / service. */
...
function renderSectionV(insp: BobcatInspection): string {
  // The screen never passes multi-signer records to the old builder, so the
  // signed-records branch was always empty — only the legacy single-signature
  // blob path is reachable. Reproduced faithfully here.
  const sigDataUrl = insp.inspectorSignature
  ...
  const sigCellsHtml = `
      <div class="sig-cell">
        ...
        <div class="sig-name">${escapeHtml(insp.inspectorName) || '—'}</div>
...
export const bobcatSchema: InspectionSchema<BobcatInspection> = {
  ...
  blocks: [
    { kind: 'custom', render: (d: BobcatInspection) => renderSectionI(d) },
    { kind: 'custom', render: (d: BobcatInspection, photos: PhotoMap) => renderSectionIII(d, photos) },
    { kind: 'custom', render: (d: BobcatInspection, photos: PhotoMap) => renderSectionIV(d, photos) },
    { kind: 'custom', render: (d: BobcatInspection) => renderSectionV(d) },
  ],
};
