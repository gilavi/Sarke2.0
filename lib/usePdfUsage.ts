...
export function usePdfUsage() {
  ...
  return useQuery<PdfUsage>({
    ...
    queryFn: async () => {
      ...
      const expiresAt = data.subscription_expires_at ? new Date(data.subscription_expires_at as string) : null;
...
/** Call after a successful PDF generation to re-sync the usage counter. */
