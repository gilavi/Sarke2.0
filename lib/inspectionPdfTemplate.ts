/**
 * inspectionPdfTemplate.ts — single-source HTML builder for generic
 * inspection PDFs (harness, bobcat-generic, etc.).
 *
 * Pure synchronous function — zero platform-specific deps.
 * Accepts pre-resolved photo URLs/data-URIs so it works identically on mobile
 * (expo-print, base64 embeds) and web (Supabase signed HTTPS URLs).
 *
 * Called by:
 *   - lib/pdf.ts          → mobile wrapper (async, embeds photos as base64)
 *   - InspectionPrint.tsx → web wrapper  (React query, signed URLs, iframe)
 */
import type {
  Answer,
  AnswerPhoto,
...
// PDF output is locked to Georgian.
