/**
 * PDF HTML generator for ტვირთის მიმღები პლატფორმის შემოწმების აქტი.
 *
 * Layout:
 *   Bilingual header →
 *   Section I  (general info 2-col grid) →
 *   Section II (platform ID table) →
 *   Section III (cargo table + total) →
 *   Section IV (checklist; fix rows amber border; N/A rows gray) →
 *   Section V  (verdict block + comment) →
 *   Section VI (photos 2-col) →
 *   Section VII (two signature blocks side-by-side + legal note)
 *
 * Fix rows: amber left border (fixable, not rejected — unlike other templates).
...
export async function buildCargoPlatformPdfHtml(args: {
  ...
  for (const entry of CP_ITEMS) {
    if (entry.section !== currentSection) {
      ...
      checklistRows += `
        <tr class="cl-cat-row">
          <td colspan="4">${escHtml(CP_SECTION_LABELS[entry.section])}</td>
        </tr>
      `;
    }
