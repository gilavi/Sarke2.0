 ...
 * and on reconnect (lib/offline.tsx, with `debounce: true`). Both list fetches
 * dedupe against the warm-up's in-flight prefetches of the same keys.
 *
 * `opts.debounce` (reconnect calls only): skip the run entirely when ANY warm
 * ran in the last 10 minutes. Field connectivity flaps constantly
 * (wifi↔cellular handoffs, dead zones), and while the per-template question
 * prefetches are already deduped by their 12h staleTime, every un-debounced
 * run still re-fetches the two lists once the 5-min staleTime lapses. The
 * post-login path never debounces (a fresh install must warm even right after
 * a reconnect blip) but does stamp the timestamp, so a blip right after login
 * is a no-op.
 */
const FLOW_WARM_DEBOUNCE_MS = 10 * 60 * 1000;
let lastFlowWarmAt = 0;

export function prefetchFlowStartCaches(qc: QueryClient, opts?: { debounce?: boolean }): void {
  if (opts?.debounce && Date.now() - lastFlowWarmAt < FLOW_WARM_DEBOUNCE_MS) return;
  lastFlowWarmAt = Date.now();
...
// generic + all 9 equipment-type tables. The project-detail screen and the
// per-project "all inspections" list both read this single query instead of
// firing per-type `listByProject` fetches.
...
export function useCertificateCounts(inspectionIds: string[]) {
  // One stable fingerprint element instead of spreading the ids into the key:
  // 50 spread ids bloat every dehydrated cache entry, and the old
  // `inspectionIds.sort()` mutated the caller's (memoized) array in place.
  const idsKey = [...inspectionIds].sort().join(',');
  return useQuery<Record<string, number>>({
    queryKey: ['certificates', 'counts', idsKey] as const,
