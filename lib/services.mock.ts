...
// Remote signing — mock-mode stub. Stores requests in memory only; the
// "send link" flow is exercised manually in real-mode against Supabase.
...
export const remoteSigningApi = {
  ...
  markSent: async (id: string): Promise<void> => {
    ...
    if (r) {
      r.status = 'sent';
      r.last_sent_at = new Date().toISOString();
    }
  },
