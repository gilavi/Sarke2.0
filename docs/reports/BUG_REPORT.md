...
## P1 — Bobcat/excavator conclusion "summary photos" never persisted to the DB · FIXED 2026-07-07

**Repro:** Open a **bobcat** (ან large-loader) inspection, reach the conclusion step, add one or more summary photos, then navigate away and reopen. The photos are gone — absent from the completed detail view, the conclusion step, and the generated PDF. On **excavator** the photos survived a reopen *on the same device* but were invisible on any other device and on the web dashboard.

**Root cause:** Both routes wrote the summary-photo path list to **AsyncStorage** (`<type>-wizard:<id>:summaryPhotos`) instead of the `summary_photos` DB column. Bobcat never read the key back at all (no `getItem`) and `lib/bobcatService.ts` didn't map the column — `toModel` never populated `summaryPhotos`, `toDb` never wrote it — so `inspection.summaryPhotos` was always `undefined` after a reload. Excavator did read the key back on load, so the list survived locally, but it too never reached the DB column, so the photos couldn't sync. The six other equipment types (forklift, cargo-platform, safety-net, mobile-ladder, general-equipment, lifting-accessories) all persist a `summary_photos` column correctly — bobcat/excavator were the two stragglers. **The DB column already existed** for both tables (added in [`0037_summary_photos_bobcat_excavator.sql`](../../supabase/migrations/0037_summary_photos_bobcat_excavator.sql) and verified present on live), so **no new migration was needed** — the defect was purely in the TypeScript mapping/route layer.

**Fix:**
- [`lib/bobcatService.ts`](../../lib/bobcatService.ts) + [`lib/excavatorService.ts`](../../lib/excavatorService.ts): added `summary_photos` to `DbRow`, `toModel` (`Array.isArray(row.summary_photos) ? … : []`), the patch type, `toDb` (`summaryPhotos → summary_photos`), and `createColumns` (`summary_photos: []`) — matching the six correct siblings.
- [`app/inspections/bobcat/[id].tsx`](../../app/inspections/bobcat/%5Bid%5D.tsx) + [`app/inspections/excavator/[id].tsx`](../../app/inspections/excavator/%5Bid%5D.tsx): summary-photo add/delete now `scheduleSave` (DB autosave) instead of `AsyncStorage.setItem`; both routes `toPatch` now include `summaryPhotos`. Removed the dead, never-wired per-item `handleAddPhoto`/`handleDeletePhoto` handlers from **both** routes (leftovers from when per-row checklist photos were dropped) plus the now-orphaned `uploadPhoto` method on each service's exported API.
- **No data loss on upgrade:** a one-time recovery hook, [`lib/inspection/useLegacySummaryPhotoRecovery.ts`](../../lib/inspection/useLegacySummaryPhotoRecovery.ts), reads the legacy AsyncStorage key on load and, when the DB row has no summary photos, patches the orphaned list into `summary_photos` (the uploaded blobs were already in the `answer-photos` bucket) then clears the key. The equipment child tables carry no freeze trigger (see [`20260623150000_allow_inspection_reopen.sql`](../../supabase/migrations/20260623150000_allow_inspection_reopen.sql)), so recovery works even for already-completed inspections; the write targets the child table only, never the frozen parent `public.inspections` row.

## P3 — PDF-limit banner missing on bobcat + general-equipment result screens · FIXED 2026-07-07

**Repro:** As a free-tier user who has hit the PDF limit, open a completed **bobcat** or **ტექ. აღჭურვილობა (general-equipment)** inspection. The other seven equipment types show an in-shell "PDF locked" banner proactively; bobcat and general-equipment showed nothing until you tapped share and the `SubscriptionNotice` modal popped. Surfaced by the 2026-07-07 architecture audit (finding F3).

**Root cause:** both routes already destructured `pdfLocked` + `limitNoticeVisible` from `useInspectionFlow` and rendered the `SubscriptionNotice` modal, but never passed the `banner` prop to `InspectionShell` — a copy-omission when the banner was added to the sibling routes. (Harness is **not** affected: a completed harness renders on the shared act detail page, which gates via `useActResult` → `pdfLocked: pdfUsage?.isLocked`.)

**Fix:** [`app/inspections/bobcat/[id].tsx`](../../app/inspections/bobcat/%5Bid%5D.tsx) + [`app/inspections/general-equipment/[id].tsx`](../../app/inspections/general-equipment/%5Bid%5D.tsx): added the `PdfLockedBanner` import and `banner={pdfLocked ? <PdfLockedBanner onDetails={() => setLimitNoticeVisible(true)} /> : undefined}` to `InspectionShell`, matching the seven other equipment routes.
...
**Follow-up · FIXED 2026-06-25 (record widgets):** the 2026-05-27 fix only warmed `['projects','list']`. Home's five cross-project record widgets (`HomeRecordsSection` → `useRecentInspections/Reports/Orders/Incidents/Briefings`, keyed `{ status: 'completed', limit: 50 }`) hit the **identical** JWT-propagation race but were never added to the warm-up, so Home showed projects but no record lists after first login — and pull-to-refresh couldn't recover them because [`app/(tabs)/home.tsx`](../../app/(tabs)/home.tsx)'s `RefreshControl` only refetched `[certsQ, templatesQ, projectsQ]`, never the widget queries (which live inside `HomeRecordsSection`). Fix: (1) [`lib/apiHooks.ts`](../../lib/apiHooks.ts) — new `warmHomeCaches(qc)` force-refetches every Home query (projects, qualifications, templates + the five record feeds) with `staleTime: 0` under the exact widget keys; [`lib/session.tsx`](../../lib/session.tsx) calls it in place of the projects-only prefetch. (2) Home's pull-to-refresh now calls the (now-awaitable) `invalidateRecordLists(qc)`, so the records reload on pull and the spinner gates on them. Covered by `tests/unit/warmHomeCaches.test.ts`.
