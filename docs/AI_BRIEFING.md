...
| Empty-state flashes on a screen the user has data on | Use `useListLoadState(q, count)` from `hooks/useListLoadState.ts` (`'data' \| 'skeleton' \| 'offline' \| 'empty'`), not `q.isLoading`/`q.isPending` and not the old inline `(isFetching \|\| !isFetched)` recipe (now banned by check-primitives — it skeletons forever on an offline paused query). For user-scoped queries also add a `staleTime: 0` prefetch in `lib/session.tsx` so a racy empty value from a previous boot can't outlive login. See `app/(tabs)/home.tsx`, `app/(tabs)/projects.tsx` for the canonical wiring. |
| Screen hangs on skeleton / blank when offline | React Query's `onlineManager` is NetInfo-wired (`lib/queryClient.ts`), so offline queries sit at `fetchStatus: 'paused'`. Any gate on `isFetched` alone never settles offline — treat `paused` as settled (see `components/FlowProjectPicker.tsx`) and render `components/OfflineEmptyState.tsx` for paused-uncached lists. Session boot is offline-first via `lib/sessionBootstrap.ts` (stored session + `@profile:<id>` cache). |
