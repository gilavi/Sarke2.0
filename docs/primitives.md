...
| [components/wizard/StepBar.tsx](../components/wizard/StepBar.tsx) | `StepBar` | Dot + connector step indicator for multi-step flows with named steps. Pass `step` (0-based) and `stepLabels` (label per step). **Currently unused** (no runtime importers; not showcased in Storybook) — kept as a candidate for future multi-step flows. |
