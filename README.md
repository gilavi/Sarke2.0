...
1. **Storage RLS — resolved (2026-05-26).** The permissive dashboard `sarke_*` policies on `certificates`, `answer-photos`, `pdfs`, `signatures` were replaced with owner-scoped policies in migration [`0053_storage_rls_owner_scoping.sql`](supabase/migrations/0053_storage_rls_owner_scoping.sql) (`owner = auth.uid()` on SELECT/UPDATE/DELETE; INSERT stays auth-only). All four buckets are now `public = false`. Read paths migrated to short-lived signed URLs across mobile + web-app (commit `618655a`). Out-of-band orphan-file cleanup is the only follow-up — tracked in `BUG_REPORT.md` as a P3.
