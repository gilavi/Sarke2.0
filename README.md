...
| `features/` | Feature-sliced modules. Each subfolder (`inspection-wizard/`, `order-new/`, `project-detail/`, `signatures/`, `records/`, `home-records/`, `history/`, `drafts/`) owns a self-contained flow with its own `AGENTS.md`; `app/` route files delegate to the matching feature module. The `records/` module owns the shared per-type "record section" building blocks (the `RecordWidget`, the status-free rows, and the 5-type descriptor) reused by `home-records/` (Home widgets), `history/` (the type-filtered History screen at `app/history.tsx`), the `drafts/` screen (`app/drafts.tsx`), and the project-detail sections. The `signatures/` module owns the unified inspection signatures flow (one creator capture + N empty hand-sign slots) and enforces the no-persistence rule for captured signature data. |
...
**Deploy gating & serialization.** Every gh-pages deploy is gated so a red check blocks production: `deploy-web.yml`, `deploy-cms.yml`, and `docs.yml` each run a `checks` (typecheck) job that the deploy job `needs:` (the small `web/`, `cms/`, and `website/` bundles have no test suite, so typecheck is the gate; note `cms build`/`docusaurus build` don't fully typecheck on their own); `deploy-web-app.yml` keeps its typecheck + unit-test + Playwright-smoke gates and adds a blocking **inspection-mirror guard** (`npm run lint:inspection-guard`) that fails the build if web-app imports the mobile engine via `@root/lib/inspection` (the same guard is a required step in `ci-web-app.yml`; the broad `@root` ESLint rule stays a warning, and the full `npm run lint` remains informational until the pre-existing backlog is cleared). Each gh-pages writer (`web`, `web-app`, `cms`, `docs`, staging, preview) has its **own** per-destination concurrency group (`gh-pages-<dest>`, `cancel-in-progress: false`) so repeated pushes to the same destination serialize while different destinations still deploy in parallel. They are deliberately **not** folded into one shared group: GitHub Actions keeps only one running + one pending run per group and cancels older pending runs, so a single shared group would silently drop sibling deploys whenever 3+ coincide (e.g. a `lib/**` change firing both `docs.yml` and `deploy-web-app.yml`, or concurrent pushes). A rare cross-destination race on the `gh-pages` ref instead surfaces as a visible, re-runnable failure.
...
| `20260623020000_harness_template_general_photos.sql` | Adds the missing `photo_upload` question ("ქამრების საერთო ფოტოები", Section 3) to the harness system template (`22222222-…`) and moves its freetext conclusion Section 3 → 4, so the inspection wizard's conclusion step shows the photo-upload section like every other template. Idempotent. **Apply manually**. |
| `20260623030000_mobile_ladder_summary_photos.sql` | Adds `summary_photos jsonb NOT NULL DEFAULT '[]'` to `mobile_ladder_inspections` so the conclusion step can carry general photos, matching the other equipment flows. Additive/nullable — existing rows unaffected. **Apply manually**. |
| `20260623150000_allow_inspection_reopen.sql` | Document-edit feature: relaxes the 0008/0010 freeze on `inspections` so an owner can **reopen** a completed generic inspection (status `completed`→`draft` + `completed_at`→null) for editing, while a row that stays completed is still fully frozen; and tightens `advance_schedule_on_complete()` to advance only on a true draft→completed rising edge, so re-completing a reopened inspection no longer double-advances `schedules.next_due_at`. Function-only (no schema change). **Apply manually**. |
| `20260625120000_drop_breathalyzer_logs_duplicate_user_fk.sql` | Fixes in-app account deletion: drops the redundant **NO ACTION** `breathalyzer_logs_user_id_fkey` (left over from `0048`) that duplicated the later ON DELETE CASCADE FK on the same column. Postgres enforced both, so a user with any breathalyzer log could not be deleted (the `delete-account` Edge Function 500'd → "Edge Function returned a non-2xx status code"). **Apply manually**. |
| `20260625130000_sync_equipment_inspection_parent_status.sql` | One-time backfill: syncs each completed/reopened `<type>_inspections` row's `status`/`completed_at` onto its shared parent `public.inspections` row. Equipment completion only wrote the equipment table, so the parent stayed `draft` and completed equipment acts never appeared in the unified feeds (Home/History `inspectionsApi.recent`, `get_project_inspections_unified`). Ongoing sync now happens app-side in `lib/inspection/service.ts`. **Apply manually**. |
| `20260701120000_risk_assessments.sql` | `risk_assessments` table — per-project **რისკების შეფასება** register; one row per document, `doc_type` ∈ {`risk_assessment`, `ppe_determination`}; `header`/`entries`/`signatories` JSONB; owner-RLS. Backs `features/risk-assessment/` + `lib/riskAssessmentService.ts`. **Apply manually** before the feature works. |
| `20260708120000_lean_list_feeds.sql` | Network-lean list feeds: `briefings_list_lean` + `orders_list_lean` views (`security_invoker`, so table RLS applies) serve the list/recent queries with signature payloads stripped — briefing `participants[].signature` / `inspector_signature` nulled, order `form_data` minus its base64 signature keys — and `get_certificate_counts(uuid[])` RPC replaces the fetch-every-cert-row client-side count. Detail/PDF paths (`getById`) keep reading the base tables; clients fall back to the legacy queries until this is applied (`lib/briefingsApi.ts`, `lib/ordersApi.ts`, `lib/services/real/qualifications.ts`). **Apply manually**. |
...
2. **Offline queue is lost on uninstall.** Offline photo capture is queued under `documentDirectory/offline-photos/` and flushes on reconnect. If the user uninstalls the app before reconnecting, the queue is gone.
   ...
   *Offline mode phase 1 (2026-07-02, `offline-mode` branch):* the app now **boots and renders offline** — `SessionProvider` restores the stored session + a cached user profile instead of hanging on `getSession()`'s token refresh, React Query's `onlineManager` is wired to NetInfo (offline queries pause instead of erroring), the persisted query cache window is 7 days, and list screens show a dedicated offline empty state via `hooks/useListLoadState.ts`. Remaining limitations: data never fetched while online is not available offline, and signing in requires a network.

   *Offline mode phase 4 (2026-07-08):* the 9 typed equipment inspection wizards (bobcat, excavator, …) now **edit, photograph, and complete offline** — autosaves/completions queue as `equipment_patch` outbox ops and photos stage to disk as `file_upload` ops inside `makeInspectionService` (see [docs/WHATS_NEW.md](docs/WHATS_NEW.md)). Reopening a completed act and PDF sharing still require a network.
