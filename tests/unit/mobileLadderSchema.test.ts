...
function mockInsp() {
  ...
  // FIXME(merge-2026-05-27): 'critical' isn't a valid MLResult literal
  // ('safe' | 'damaged' | 'na'). Cast keeps the original test compilable.
  insp.items = insp.items.map((it, i) =>
    i === 0
      ? { ...it, result: 'safe', photo_paths: ['ml/x/1.jpg'] }
      : i === 1
        ? { ...it, result: 'critical' as any, comment: 'გაუმართავია' }
        : it,
  );
  insp.verdict = 'safe';
  ...
  // FIXME(merge-2026-05-27): summaryPhotos isn't on MobileLadderInspection.
  (insp as any).summaryPhotos = ['ml/x/sum.jpg'];
