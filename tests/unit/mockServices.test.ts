...
describe('mock signaturesApi', () => {
  it('list + upsert (insert and update branches) + remove', async () => {
    expect(await signaturesApi.list('insp-1')).toEqual([]);
    const s = await signaturesApi.upsert({
      inspection_id: 'insp-1',
      signer_role: 'expert',
      full_name: 'Gio',
      phone: null,
    ...
    const s2 = await signaturesApi.upsert({
      inspection_id: 'insp-1',
      signer_role: 'expert',
      full_name: 'Gio Updated',
      phone: null,
      position: null,
      signature_png_url: 'data:image/png;base64,YY',
      status: 'signed',
      person_name: null,
    });
    expect(s2.id).toBe(s.id);

    await signaturesApi.remove('insp-1', 'expert');
    expect(await signaturesApi.list('insp-1')).toEqual([]);
  });
});
