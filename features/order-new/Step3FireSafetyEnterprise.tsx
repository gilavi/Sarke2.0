    ...
    <View style={{ gap: 12 }}>
      ...
      <View onLayout={registerField('appointedPosition')}>
        <FloatingLabelInput
          ...
          error={attempted && !form.appointedPosition.trim() ? t('orders.requiredField') : undefined}
        />
      </View>
      <View onLayout={registerField('appointedIdNumber')}>
        <FloatingLabelInput
          label={t('orders.personalId')}
          required
          value={form.appointedIdNumber}
          onChangeText={v => setForm(f => ({ ...f, appointedIdNumber: v }))}
          error={attempted && !form.appointedIdNumber.trim() ? t('orders.requiredField') : undefined}
