...
const severityStyles: Record<string, { class: string; label: string }> = {
  critical: { class: 'sg-badge--critical', label: 'კრიტიკული' },
  high: { class: 'sg-badge--high', label: 'მაღალი' },
  medium: { class: 'sg-badge--medium', label: 'საშუალო' },
  low: { class: 'sg-badge--low', label: 'დაბალი' },
};

const categoryStyles: Record<string, { class: string; label: string }> = {
  hazard: { class: 'sg-badge--hazard', label: 'საფრთხე' },
  ppe: { class: 'sg-badge--ppe', label: 'PPE' },
  procedure: { class: 'sg-badge--procedure', label: 'პროცედურა' },
  compliance: { class: 'sg-badge--compliance', label: 'სტანდარტი' },
};
  ...
  if (!isPanelOpen) {
    return (
      <button
        ...
        aria-label="უსაფრთხოების პანელის გახსნა"
      >
    ...
    <div className="sg-panel">
      <div className="sg-panel__header">
        <div className="sg-panel__title">
          ...
          <div>
            <h2>უსაფრთხოების სახელმძღვანელო</h2>
            <p>{tip ? 'ნაწილი არჩეულია' : 'აირჩიეთ კონსტრუქციული ნაწილი'}</p>
          </div>
        </div>
        <button onClick={() => setPanelOpen(false)} className="sg-panel__close" aria-label="პანელის დახურვა">
      ...
      <div className="sg-panel__body">
        {!tip ? (
          <div className="sg-empty">
            ...
            <h3>ინტერაქტიული 3D უსაფრთხოების სახელმძღვანელო</h3>
            <p>დააჭირეთ 3D სცენაში ნებისმიერ კონსტრუქციულ ელემენტს დეტალური უსაფრთხოების ინსტრუქციების, საფრთხის ინფორმაციისა და სტანდარტების მოთხოვნების სანახავად.</p>
            ...
            <div className="sg-content__header">
              ...
              <div className="sg-badges">
                <span className={`sg-badge ${severityStyles[tip.severity].class}`}>
                  სიმძიმე: {severityStyles[tip.severity].label}
                </span>
