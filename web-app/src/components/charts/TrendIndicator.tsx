...
export function TrendIndicator({ current, previous, label }: TrendIndicatorProps) {
  ...
  return (
    <div className="flex items-center gap-1 text-xs">
      {isUp ? <ArrowUp size={12} className="text-green-500" /> : <ArrowDown size={12} className="text-red-500" />}
      <span className={isUp ? 'text-green-600' : 'text-red-600'}>{Math.abs(diff).toFixed(0)}%</span>
