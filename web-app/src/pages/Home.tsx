...
export default function Home() {
  ...
  const { data: inspections, isLoading: l1 } = useQuery({ queryKey: ['inspections'], queryFn: () => listInspections() });
  ...
  const { data: projects, isLoading: l5 } = useQuery({ queryKey: ['projects'], queryFn: listProjects });
  const { data: incidents, isLoading: l6 } = useQuery({ queryKey: ['incidents'], queryFn: () => listIncidents() });
  const { data: briefings, isLoading: l7 } = useQuery({ queryKey: ['briefings'], queryFn: () => listBriefings() });
