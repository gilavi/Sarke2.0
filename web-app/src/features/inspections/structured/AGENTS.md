...
- `steps/` — `SpecStep`, `ChecklistStep`, `VerdictStep` presentationals.
  `ChecklistStep` renders the **Audit Ledger**: one grid row per item
  (`[mono number] · [title + muted subtitle, fills width] · [seg control, fixed
  width, right-aligned]`) with a hairline divider between rows and no per-item
  cards. Groups (A/B…) get a mono uppercase letter-spaced muted eyebrow above the
  group. Status is the shared monochrome `SegmentedControl` (ink-filled selected
  segment, label carries meaning — no green/red/amber). The header also shows a
  mono `N / M შემოწმებული` counter for checklist steps.
