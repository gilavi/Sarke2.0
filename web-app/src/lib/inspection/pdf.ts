 ...
 * inspection data, and a pre-resolved PhotoMap (https URLs on web) and returns a
 * complete HTML document. Web-app mirror of the Expo app's
 * `lib/inspection/pdf.ts` (the `@root` import is banned by eslint).
 *
 * Photos are pre-resolved by the print route (signed Supabase URLs) before being
 * handed in here as a PhotoMap.
 */
...
function resolveField<T>(v: string | ((d: T) => string) | undefined, d: T): string | undefined {
  return typeof v === 'function' ? (v as (d: T) => string)(d) : v;
}
